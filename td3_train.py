...
from furuta_gym.envs.common import GentlyTerminating
...
def main(args):
    ...
    experiment_name, writer = setup_tracking(args)
    env = setup_env(args, experiment_name)
    seed(env)
    rb, actor, qf1, qf2 = setup_model(args, env, device)
    train(args, env, rb, actor, qf1, qf2, device, writer, experiment_name)
...
def setup_env(args, experiment_name):
    ...
    if args.capture_video:
        env = Monitor(env, f'videos/{experiment_name}',
                      video_callable=lambda x: x%5 == 0)  # log video every five episode
...
def setup_model(args, env, device):
    ...
    if args.model_artifact is not None:
        ...
        actor.load_state_dict(torch.load(os.path.join(artifact_dir,
                                         "target_actor.pth")))
...
def train(args, env, rb, actor, qf1, qf2, device, writer, experiment_name):
    ...
    try:
        for global_step in range(args.total_timesteps):
            ...
            if global_step < args.policy_starts:
                ...
                # doing this bc it seems like the first time
                # we run this it takes forever??
                if not first_action_done and args.gym_id == "Furuta-v0":
                    logging.debug("First action hack")
                    env.motor.set_speed(0)
                    first_action_done = True
                action = actor.forward(obs.reshape((1,)+obs.shape), device)
                action = (
                    action.tolist()[0]
                    + np.random.normal(0, max_action * args.exploration_noise,
            ...
            # make sure loop time/dt is constant
            # (speed calculation are based on fixed dt)
            dt_diff = env.timing.dt_ctrl - real_dt
            if dt_diff < 0 or args.gym_id != "FurutaReal-v0":
                # bad news the loop is too slow (or we're in sim)
                # TODO throw error or warning
                pass
            else:
                time.sleep(dt_diff)

            # TODO replace by summary writer for coherence
            wandb.log({**{"loop_time": real_dt,
                       "dt_diff": dt_diff}, **info})
        ...
        logging.info("Interrupting training")
        env.close()
    ...
    model_dir = f"runs/{experiment_name}/models/"
    ...
    torch.save(target_actor.state_dict(),
               os.path.join(model_dir, "target_actor.pth"))
    artifact = wandb.Artifact("td3_model", type="model")
    artifact.add_dir(model_dir)
    wandb.log_artifact(artifact)

    logging.info("Saving replay buffer")
    rb_dir = f"runs/{experiment_name}/replay_buffer/"
    os.makedirs(rb_dir)
...
def parse_args():
    ...
    parser.add_argument('--torch-deterministic',
                        ...
                        help='Sets `torch.backends.cudnn.deterministic`')
    ...
    parser.add_argument('--policy-frequency',
                        type=int, default=2,  # relative to training frequency
                        help="the frequency of training policy (delayed)")
    parser.add_argument('--noise-clip', type=float, default=0.5,
                        help='noise clip parameter of the \
    ...
    parser.add_argument('--max_steps', type=int, default=3000,
                        help='Max steps before terminating episode')
