...
from furuta.rl.wrappers import (
    ControlFrequency,
    GentlyTerminating,
    HistoryWrapper,
    MCAPLogger,
...
def main(args):
    ...
    if args.model_artifact:
        model.load(download_artifact_file(f"sac_model:{args.model_artifact}", "sac.zip"))

    if args.rb_artifact:
        rb_path = download_artifact_file(f"sac_replay_buffer:{args.model_artifact}", "buffer.pkl")
...
def setup_env(args):
    ...
    if args.log_mcap:
        env = MCAPLogger(
            env, f"../data/{wandb.run.id}", use_sim_time=(args.gym_id == "FurutaSim-v0")
        )
...
def parse_args():
    ...
    # Common arguments
    parser.add_argument(
        "--gym_id", type=str, default="FurutaSim-v0", help="the id of the gym environment"
    )
    parser.add_argument(
        "--learning_rate", type=float, default=3e-4, help="the learning rate for the optimizer"
    )
    parser.add_argument("--seed", type=int, default=1, help="seed of the experiment")
    parser.add_argument(
        "--total_timesteps", type=int, default=1000000, help="total timesteps of the experiments"
    )
    parser.add_argument(
        "--capture_video",
    ...
    )  # TODO make that an an int n and if = 0 then no video
    # else it captures every n steps
    parser.add_argument(
        "--log_mcap", type=lambda x: bool(strtobool(x)), default=False, help="log mcap data"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--buffer_size", type=int, default=int(1e6), help="the replay memory buffer size"
    )
    parser.add_argument("--tau", type=float, default=0.005, help="target smoothing coefficient.")
    parser.add_argument("--gamma", type=float, default=0.99, help="target smoothing coefficient.")
    parser.add_argument(
        "--batch_size",
        type=int,
        default=256,
    ...
    parser.add_argument("--learning_starts", type=int, default=25e3, help="when to start learning")
    parser.add_argument(
        "--sde_sample_freq",
        type=int,
        default=-1,
    ...
    parser.add_argument(
        "--train_freq", type=int, default=1, help="The frequency of training critics/q functions"
    )
    parser.add_argument(
        "--train_freq_unit", type=str, default="episode", help="The frequency unit"
    )
    parser.add_argument(
        "--gradient_steps", type=int, default=-1, help="How many training iterations."
    )
    ...
    parser.add_argument(
        "--safety_th_lim", type=float, default=1.5, help="Max motor (theta) angle in rad."
    )
    parser.add_argument(
        "--action_limiter",
        type=lambda x: bool(strtobool(x)),
    ...
    parser.add_argument("--history", type=int, default=1, help="If >1 use HistoryWrapper")
    parser.add_argument(
        "--custom_sim", type=str, default=None, help="Use params from the provided file."
    )
